    ...
    for (let i = 0; i < row; i++) {
      for (let j = 0; j < col; j++) {
        const temp = {
          ...
          sx: randomNum(-step_row / 2, step_row / 2), // 当前偏移量x
          sy: randomNum(-step_col / 2, step_col / 2), // 当前偏移量y
          dx: !!Math.round(randomNum(0, 1)), // 当前方向x
          dy: !!Math.round(randomNum(0, 1)), // 当前方向y
          color: randomNum(20, 70), // b通道颜色值
          dcolor: !!Math.round(randomNum(0, 1)), // 颜色改变向量
        };
        this.dots.push(temp);
      }
    }
